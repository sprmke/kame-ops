...
| `receipts`              | list, unpaidDueEntries, **getUploadProgress**, validateAndMarkPaid, confirmMarkPaid, **revalidateWithAi**, **delete**                                                                                                                       | Implemented — AI validation, mark SOA paid, upload progress; month view groups by SOA statement period; group header paid summary counts receipts in section        |
...
| `GET /api/cron/dispatch`      | Cron: run due `automation_jobs` + automatic payment reminders for eligible users (Bearer `CRON_SECRET`) | Implemented |
...
| `automation.service.ts`         | Job CRUD, dispatch, run-now; cron via `/api/cron/dispatch`; `send_due_reminders` updates lock schedule to daily (time only)                    |
