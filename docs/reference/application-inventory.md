...
| `receipts`              | list, unpaidDueEntries, **getUploadProgress**, validateAndMarkPaid, confirmMarkPaid, **revalidateWithAi**, **delete**                                                                                                                       | Implemented — AI validation, partial/multi-receipt mark paid (sums receipts per due entry), upload progress completes for partial payments; month view groups by SOA statement period; group header paid summary counts receipts in section        |
