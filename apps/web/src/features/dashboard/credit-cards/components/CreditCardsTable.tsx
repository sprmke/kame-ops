  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border/80">
      <Table>
        ...
        <TableBody>
          {cards.map((card) => (
            <ClickableTableRow key={card.id} onRowClick={() => onEdit(card)}>
              <TableCell className="font-medium">
                <span className="font-display">
                  {card.label ?? formatBankIssuer(card.issuer)}
                </span>
              </TableCell>
              <TableCell>{formatBankIssuer(card.issuer)}</TableCell>
              <TableCell className="tabular-nums">•••• {card.last4}</TableCell>
              <TableCell>
                <StatusBadge
